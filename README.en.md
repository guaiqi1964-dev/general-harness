...
Download `General-Harness-Setup-0.2.0.exe`, double-click and follow the wizard:
...
Download `General-Harness-v0.2.0.zip` and extract it to any directory — no installation needed. The engine is a single zero-dependency file; double-click `start.bat` or run `bin\gh_upx.exe` directly.
